...
这篇论文最值得关注的在于，它把推 reasoning 从生成过程中单独拿了出来。这样一来，优化目标就不再只是图像结果，而是中间的 reasoning chain 本身。这个思路其实很像 agent 里的 planning 和 reflection，也说明生成模型往后走，很可能不只是拼生成质量，而是会越来越强调中间推理过程能不能被显式建模和优化。